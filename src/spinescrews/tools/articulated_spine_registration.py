...
def _save_cmaes_inputs(spine, postop_pts, icp_affs, ratios, kdtrees,
    ...
    """Dump all _cmaes_refit inputs to disk for offline debugging."""
    import json
    d = _CMAES_DEBUG_DIR
...
def _cmaes_refit_deprecated(spine, postop_pts, icp_affs, ratios, kdtrees,
                            postop_img=None, artifact_mask=None, metal_thresh=2000,
                            level_names=None, iso_res=1.5, initial_radius=12.):
    """DEPRECATED: replaced by _perlevel_refit. Kept for reference.

    Re-optimize vertebral transforms jointly with CMA-ES, freezing well-aligned levels.
    """
    import cma
    ...
    # Coarse-to-fine: sparse points + wide GM for exploration, dense + tight GM for polish
    n_jobs = max(1, os.cpu_count() - 2)
    postop_tree_full = KDTree(postop_pts)
    total_landmarks = sum(len(lm) for lm in spine.landmarks)
    N = min(len(postop_pts), total_landmarks)
    stages = [(N // 4, 5.0), (N // 2, 3.0), (N, 2.0)]

    # All levels start free; frozen set populated after stage 0
    free_idx = np.arange(nJ)
    frozen_params = init_params.copy()
    x0_full = init_params.copy()
    sensitivity = np.zeros(nJ)  # per-level gradient magnitude (updated after each stage)
    sigma0 = 1.0

    with timed('CMA-ES refit'):
        for stage, (n_pts, gm_bp) in enumerate(stages):
            # Subsample postop + preop points
            ref_p = x0_full.reshape([nJ, 6])
            ref_affs = make_aff(ref_p[:, :3], ref_p[:, 3:])
            stride = max(1, len(postop_pts) // n_pts)
            pts = postop_pts[::stride]
            nP = len(pts)
            masks = np.zeros([nJ, nP], dtype=bool)
            for jj in range(nJ):
                preop_inv = transform_points_inverse(ref_affs[jj], pts)
                masks[jj] = kdtrees[jj].query(preop_inv)[0] < 2 * initial_radius
            lm_stride = max(1, total_landmarks // n_pts)
            query_lm = [lm[::lm_stride] for lm in spine.landmarks]

            # Extract free-level params and build stds
            n_free = len(free_idx)
            x0_free = np.concatenate([x0_full[j*6:(j+1)*6] for j in free_idx])
            stds_free = np.zeros(6 * n_free)
            for k, j in enumerate(free_idx):
                # Sensitivity-based: low sensitivity = wide search, high = narrow
                sens = sensitivity[j] if sensitivity[j] < np.inf else 0.0
                scale = 1.0 / (1.0 + sens / 0.01)  # ~1 when sens≈0, ~0.5 at threshold
                stds_free[k * 6: k * 6 + 3] = max(scale, 0.2) * 0.05   # rotation (rad)
                stds_free[k * 6 + 3: k * 6 + 6] = max(scale, 0.2) * 3.0  # translation (mm)

            # Cost: map free params → full params → _spine_cost
            def cost(free_params, _frozen=frozen_params.copy(), _fidx=free_idx,
                     _masks=masks, _pts=pts, _tree=postop_tree_full,
                     _qlm=query_lm, _c=gm_bp):
                full = _frozen.copy()
                for k, j in enumerate(_fidx):
                    full[j*6:(j+1)*6] = free_params[k*6:(k+1)*6]
                return float(np.sum(_spine_cost(
                    full, spine.default_aff, kdtrees, _masks, _pts,
                    gm_breakpoint=_c, postop_tree=_tree, query_landmarks=_qlm)**2))

            maxiter = 400 * (len(stages) - stage)
            n_query_lm = sum(len(lm) for lm in query_lm)
            log.info('CMA-ES stage %d: %d d2m pts, %d m2d lm, %d free (%d/%d levels), maxiter %d',
                     stage, nP, n_query_lm, 6 * n_free, n_free, nJ, maxiter)

            # Run CMA-ES
            opts = cma.CMAOptions()
            opts['CMA_stds'] = stds_free.tolist()
            opts['CSA_dampfac'] = 4  # slower sigma adaptation
            opts['verbose'] = -9
            opts['tolfun'] = 1e-6   # effectively disabled; rely on tolx + maxiter
            opts['tolx'] = 0.01
            opts['maxiter'] = maxiter
            es = cma.CMAEvolutionStrategy(x0_free, sigma0, opts)
            gen = 0
            init_cost = cost(x0_free)
            while not es.stop():
                X = es.ask()
                fitvals = Parallel(n_jobs=n_jobs, prefer='threads')(delayed(cost)(x) for x in X)
                es.tell(X, fitvals)
                gen += 1
                if (gen == 1) or (gen % 50 == 0):
                    log.info('CMA-ES stage %d iter %d: cost %.4f (sigma %.3f)',
                             stage, gen, es.result.fbest, es.sigma)
            xopt_free = es.result.xbest
            opt_cost = cost(xopt_free)
            log.info('CMA-ES stage %d: cost %.4f -> %.4f in %d iters (sigma %.3f)',
                     stage, init_cost, opt_cost, gen, es.sigma)

            # Merge free params back into full vector
            for k, j in enumerate(free_idx):
                x0_full[j*6:(j+1)*6] = xopt_free[k*6:(k+1)*6]

            prev_free = free_idx.copy()
            # Freeze gate: per-level gradient magnitude of m2d cost
            # High sensitivity = sharp basin (well-registered), low = plateau (poorly-registered)
            c2_fg = gm_bp ** 2
            eps_fg = np.array([np.radians(1.0)] * 3 + [1.0] * 3)  # 1° rot, 1mm trans
            p_fg = x0_full.reshape([nJ, 6])
            affs_fg = make_aff(p_fg[:, :3], p_fg[:, 3:])
            baseline = np.empty(nJ)
            for j in range(nJ):
                d = postop_tree_full.query(transform_points_forward(affs_fg[j], spine.landmarks[j]))[0]
                baseline[j] = np.mean(d**2 / (c2_fg + d**2))

            sensitivity = np.full(nJ, np.inf)
            sens_threshold = 0.1
            for j in free_idx:
                def _cost_j(p6, _j=j):
                    aff_j = make_aff(p6[:3][None], p6[3:][None])[0]
                    d = postop_tree_full.query(transform_points_forward(aff_j, spine.landmarks[_j]))[0]
                    return np.mean(d**2 / (c2_fg + d**2))
                grad = approx_fprime(x0_full[j*6:(j+1)*6], _cost_j, eps_fg)
                sensitivity[j] = float(np.linalg.norm(grad))

            freeze_mask = np.ones(nJ, dtype=bool)
            freeze_mask[free_idx] = sensitivity[free_idx] <= sens_threshold
            free_idx = np.where(~freeze_mask)[0]
            frozen_params = x0_full.copy()
            sigma0 = 1.0  # reset — converged sigma is too small for free levels

            def _names(idx):
                return [level_names[j] if level_names else str(j) for j in idx]
            for j in prev_free:
                name = level_names[j] if level_names else str(j)
                log.info('  %s: sensitivity=%.4f (baseline=%.4f) → %s',
                         name, sensitivity[j], baseline[j],
                         'FROZEN' if freeze_mask[j] else 'free')
            log.info('CMA-ES freeze: free [%s], frozen [%s]',
                     ', '.join(_names(free_idx)),
                     ', '.join(_names(np.where(freeze_mask)[0])))

            if len(free_idx) == 0:
                log.info('All levels frozen — stopping early')
                break

    # Unpack optimized transforms
    opt_params = x0_full.reshape([nJ, 6])
    opt_affs = make_aff(opt_params[:, :3], opt_params[:, 3:])

    # Per-level diagnostics: ratio on refit points, translation/rotation shift
    ...
    for jj in range(nJ):
        ...
        log.info('CMA-ES %s: ratio %.3f -> %.3f (%+.3f), moved %.2fmm / %.2f°',
                 name, ratios[jj], new_ratios[jj], new_ratios[jj] - ratios[jj], d_trans, d_rot)
